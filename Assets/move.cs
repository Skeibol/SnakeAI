...
public class Move : MonoBehaviour {
	public static Action<Vector2> SendPosition;
	...
	void Start()
	{
		...
		b1 = rw.getWeights(Application.dataPath + "/b1.txt");
		b2 = rw.getWeights(Application.dataPath + "/b2.txt");
		//b3 = rw.getWeights(Application.dataPath + "/b3.txt");
		//b4 = rw.getWeights(Application.dataPath + "/b4.txt");
		W1 = rw.getWeights(Application.dataPath + "/W1.txt");
		W2 = rw.getWeights(Application.dataPath + "/W2.txt");
		//W3 = rw.getWeights(Application.dataPath + "/W3.txt");
		//W4 = rw.getWeights(Application.dataPath + "/W4.txt");
	}
	void InitValues()
	{
	...
	private float[,] forward(float[,] X_input,
		float[,] W1, float[,] W2,/* float[,] W3, float[,] W4,*/
		float[,] b1, float[,] b2/*, float[,] b3, float[,] b4*/) // SCALE WHEN CHANGING NETWORK
	{
		...
		return A2; // SCALE WHEN CHANGING NETWORK
	...
	private void FixedUpdate()
	{
		...
		_output.text = string.Format("Up: {0:f2}%<br>Down: {1:f2}%<br>Left: {2:f2}%<br>Right: {3:f2}%<br>Do nothing: {4:f2}%", dec[0, 0] * 100, dec[1, 0] * 100, dec[2, 0] * 100, dec[3, 0] * 100, dec[4, 0] * 100);
	...
	private float[,] CollectData(Vector2 direction, Vector2 prevDir)
	{
		...
		SendPosition?.Invoke(new Vector2(X * 38 - 19, Y * 18 - 9));
	...
	private float[,] Softmax(float[,] x)
	{
		...
		for (int i = 0; i < x.GetLength(0); i++)
		{
			for (int j = 0; j < x.GetLength(1); j++)
			{
				x[i, j] = Mathf.Pow((float)Math.E, x[i, j] - max);
				sum += x[i, j];
